...
void Send2LockViaParsCom(int Lk_Cmd, long value, long value2, double arg3,
			 double arg4);
void send2AllCntlrsViaParsCom(int parse_cmd, long num_acodes,
			      long num_tables, long cur_acode, char *Id);
void sendExceptionViaParsCom(int error, int errorcode, int d1, int d2,
			     char *str);
...
volatile unsigned int *fpga_linearization_enable =
    (volatile unsigned int *)(FPGA_BASE_ADR + RF_LinearizationControl);
volatile unsigned int *fpga_atten_mapping =
    (volatile unsigned int *)(FPGA_BASE_ADR + RF_AttenMappingTable);
volatile unsigned int *fpga_amp_table =
    (volatile unsigned int *)(FPGA_BASE_ADR + RF_AmpLinearizationTable);
volatile unsigned int *fpga_phase_table =
    (volatile unsigned int *)(FPGA_BASE_ADR + RF_PhaseLinearizationTable);
volatile unsigned int *fpga_tpwr_map_tbl =
    (volatile unsigned int *)(FPGA_BASE_ADR + RF_TPowerMappingTable);
volatile unsigned int *fpga_tpwr_scale_tbl =
    (volatile unsigned int *)(FPGA_BASE_ADR + RF_LinearizationScaleTable);
...
char *rt_descr[] =
    { "np", "nt", "ct", "dpts", "autop", "il_incr", "il_incrBsCnt",
	/* 7 - 12 */ "ra_fidnum", "ra_ctnum", "rtvptr", "idver", "o2auto",
	"dpf",
	/* 13 - 20 */ "oph", "bs", "bsct", "ss", "ssct", "cpf", "cfct",
	"tablert",
	/* 21 - 28 */ "rttmp", "spare1", "id2", "id3", "id4", "zero", "one",
	"two",
...
*/
int
nextScan(ACODE_ID pAcodeId, int fidnum, int nf, int nfmod, int cf,
	 int dsize, int *srctag, int *dsttag)
{
	int endct, ctmod;
	...
	if (ct % ctmod == 0) {
		...
		if ((cf % nfmod) == 0) {
			...
			p2statb = (FID_STAT_BLOCK *) allocAcqDataBlock((ulong_t) nfidnum,	// must be same id for all bs blks
								       (ulong_t) np,	// adjusted for last nfmod
								       (ulong_t) ct, (ulong_t) endct, (ulong_t) nt, (ulong_t) dsize,	// adjusted for last nfmod
			...
			DPRINT6(+2,
				"    nextScan: FID_STAT_BLOCK: 0x%lx fidnum:%ld trace:%ld tagId:%d dsize:%d maxid:%d\n",
				p2statb, fidnum, nfidnum, tagId, dsize,
				smax - 1);
		...
		if (il_flag != 1) {
			*srctag =
			    pAcodeId->dspSrcBuf + cf / nfmod +
			    (ct / ctmod) * nf / nfmod;
			DPRINT6(+5,
				"    nextScan: fidnum: %d, srctag %d = dspSrcBuf %d + (cf/nfmod) %d + (ct/ctmod) %d * (nf/nfmod) %d\n",
				fidnum, *srctag, pAcodeId->dspSrcBuf,
				(cf / nfmod), (ct / ctmod), (nf / nfmod));
		} else {
			*srctag = pAcodeId->dspSrcBuf;
			DPRINT3(+5,
				"    nextScan: fidnum: %d, srctag %d = dspSrcBuf %d \n",
				fidnum, *srctag, pAcodeId->dspSrcBuf);
		}
		...
		if (il_flag != 1) {
			*srctag =
			...
			} else	// if (nfmod < nf ) 
			{
				...
				// cf  nfmod  ct ctmod
				*srctag =
				    pAcodeId->dspSrcBuf - ((nf / nfmod) - 1) +
				    traceNum;
				DPRINT5(5,
					"    nextScan: fidnum: %d, srctag %d = dspSrcBuf %d - (tracesPerFid - 1) %d + trace#  %d\n",
					fidnum, *srctag, pAcodeId->dspSrcBuf,
					((nf / nfmod) - 1), traceNum);
			}
		}
...
//              fidnum, *srctag, pAcodeId->dspSrcBuf, (cf/nfmod), (ct/ctmod), (nf/nfmod));
	...
	DPRINT5(1,
		"    nextScan: fid:%d cf/nfmod:%d ct/ctmod:%d src:%d dst:%d\n",
...
int getTableElement(int table_index, int element, int *errorcode, char *emssg)
{
	...
	*errorcode = 0;
	...
	if ((table_index < 0) || (table_index >= MAXTABLES)) {
		errLogRet(LOGIT, debugInfo,
			  "getP2TableElement: invalid table index %d\n",
			  table_index);
...
void
putTableElement(int table_index, int element, int val, int *errorcode,
		char *emssg)
{
	unsigned int *tableList, *p2TableWord;
	int tableSize, autoIncrFlag, divNFactor, elemSize;
	...
	*errorcode = 0;
	...
	if (elemSize != 32) {
		errLogRet(LOGIT, debugInfo,
			  "TABLE: invalid elemsize for table\n");
		*errorcode = SYSTEMERROR + NOACODETBL;
		return;
	}
	/* tableList points to the first table value */
	  ...
	  int wordsPerState, int patoffset, long long patSkipTicks, int reps)
{
	...
	DecInfo.patId = patid;
	errorCode =
	    getPattern(patid, &(DecInfo.PatternList), &(DecInfo.PatternSize));
	if (errorCode != 0) {
		return (errorCode);
	}
	if (pat_ticks < 4LL)
		pat_ticks = 4LL;	/* protect divide by zero */
...
int isIncr(int w1)
{
	if ((w1 & RFPHASEKEY) == RFPHASEKEY)
		return (1);
	if ((w1 & RFPHASECYCLEKEY) == RFPHASECYCLEKEY)
		return (1);
	if ((w1 & RFAMPKEY) == RFAMPKEY)
		return (1);
	if ((w1 & RFAMPSCALEKEY) == RFAMPSCALEKEY)
		return (1);
	return (0);
}
...
int progDecOff(int patid, int policy, long long runTicks)
{
	...
	DPRINT5(-1,
		"PROG DEC OFF; tickPerState: %lld,  total %lld  cycle %lld loops %lld  policy %d\n",
		DecInfo.timeperState, totalTicks, DecInfo.patTicks, loopcount,
		DecInfo.policy);
	...
	while (temp > 0LL) {
		...
		if (offsetFlag) {
			...
			for (index = 0; index < loops; index++) {
				writeCntrlFifoWord(RFPHASECYCLEKEY |
						   ((runningPhase >> 15))
						   & 0xffff);
				runningPhase += DecInfo.phasePerPass;
				writeCntrlFifoBuf(DecInfo.PatternList,
						  DecInfo.PatternSize);
			}
		} else {
			// small pattern guard...
			if (DecInfo.PatternSize < DecThreshold) {
				...
				status =
				    sendCntrlFifoList((DecInfo.PatternList),
						      (DecInfo.PatternSize),
						      loops);
				if (status == -1) {
					errLogRet(LOGIT, debugInfo,
						  "progDecOff() : sendCntrlFifoList error!! \n");
					return (HDWAREERROR +
						PROG_DEC_LOAD_ERR);
				}
			}
	...
	if (offsetFlag)
		writeCntrlFifoWord(RFPHASECYCLEKEY | ((runningPhase >> 15)) &
				   0xffff);
	...
	if ((DecInfo.policy == DECCONTINUE)
	    && (pAcqReferenceData->ct == (pAcqReferenceData->nt - 1))) {
		DecInfo.policy = DECSTOPEND;
	}

	/* we have totalTicks time left - what do we do?? */

	if (DecInfo.policy == DECCONTINUE) {	/* we know we're continuing */
		DPRINT1(+1,
			"DECONTINUE PUSHS TICKS %lld to next PROG DEC CYCLE\n",
			totalTicks);
	...
	if (DecInfo.policy == DECSTOPEND) {
		...
		if ((wce - wcs) > 0) {
			status =
			    sendCntrlFifoList(DecInfo.PatternList + wcs,
					      wce - wcs, ONE_TIME);
		...
		while (endTicks > 0LL) {
			if (pRFWfg >
			    (DecInfo.PatternList + DecInfo.PatternSize)) {
		...
		writeCntrlFifoWord(RFPHASEKEY | 0);	/* DEFAULT NO LATCH!!! */
		if (offsetFlag)
			writeCntrlFifoWord(RFPHASECYCLEKEY | DecInfo.
					   savedPhase);
		DecInfo.remTicks = 0LL;
		DecInfo.progDecRunning = 0;
		endTicks = 0LL;
...
progDecSkipOff(int patid, int policy, long long runTicks, long long skipTicks)
{
	...
	if (skipTicks >= DecInfo.patTicks) {
		errLogRet(LOGIT, debugInfo,
			  "error in programmable decoupling async skip time too large\n");
	...
	fract_ticks =
	    (int)(DecInfo.timeperState -
		  (skipTicks - skipfull_states * DecInfo.timeperState));
	if ((fract_ticks < 0) || (fract_ticks > DecInfo.timeperState)) {
		errLogRet(LOGIT, debugInfo,
			  "error in programmable decoupling async skip calculation 1\n");
		return (HDWAREERROR + PROG_DEC_PAT_ERR);
	}
	...
	pRFWfg =
	    (DecInfo.PatternList) + (skipfull_states * (DecInfo.wordsperstate));

	if ((pRFWfg + (DecInfo.wordsperstate)) >
	    ((DecInfo.PatternList) + (DecInfo.PatternSize))) {
		errLogRet(LOGIT, debugInfo,
			  "error in programmable decoupling async skip calculation 2\n");
		return (HDWAREERROR + PROG_DEC_PAT_ERR);
	}
	...
	if (DecInfo.wordsperstate == 3) {
		if (((*pRFWfg) & 0x7C000000) == GATEKEY) {
			...
			errLogRet(LOGIT, debugInfo,
				  "error in executing first fractional waveform state\n");
	}
	...
	if (outstates > 0) {
		...
		writeCntrlFifoWord(encode_RFSetDuration
				   (0, DecInfo.timeperState));
		/* protect AGAINST SMALL PATTERNS */
		status =
		    sendCntrlFifoList((DecInfo.PatternList +
				       ((skipfull_states +
					 1) * DecInfo.wordsperstate)),
				      outstates * DecInfo.wordsperstate,
				      ONE_TIME);
		temp -= (DecInfo.timeperState) * outstates;
		totalTicks -= (DecInfo.timeperState) * outstates;
		DPRINT1(-1,
			"progDecSkipOff stage B has %lld ticks left over ???\n",
			temp);
	}
	...
	/* this can send an enormous number of loops */
	DPRINT5(-1,
		"PROG DEC SKIP OFF; tickPerState: %lld,  total %lld  cycle %lld loops %lld  policy %d\n",
		DecInfo.timeperState, totalTicks, DecInfo.patTicks, loopcount,
		DecInfo.policy);
		...
		if (endTicks > 0LL) {
			...
			if (DecInfo.wordsperstate == 3) {
				if (((*pRFWfg) & 0x7C000000) == GATEKEY) {
					...
					errLogRet(LOGIT, debugInfo,
						  "error in executing last fractional waveform state\n");
			}
			...
			/* DURATION WORD  NO LATCH */
			writeCntrlFifoWord(DURATIONKEY | (int)endTicks &
					   0x3fffff);
...
void
calc_obl_matrix(double ang1, double ang2, double ang3, double *tm11,
		double *tm12, double *tm13, double *tm21, double *tm22,
		double *tm23, double *tm31, double *tm32, double *tm33)
{
	double D_R;
...
#define MAXIFZ 5		/* max number of nested ifzeros */
	...
	p2EndOfCodes =
	    pAcodeId->cur_acode_base + (pAcodeId->cur_acode_size / sizeof(int));
	...
	while (codesOK) {
		...
		if (priorityInversionCntDwn > 0) {
			...
			*unsigned long bufDurationGet();
			...
			wvEvent((EVENT_MRISUERBYTE_CNTDWN +
				 (0xFF & priorityInversionCntDwn)), NULL, NULL);
			...
			if (((priorityInversionCntDwn % 10) == 0)
			    && (wordsinfifo > 0)) {
				flushCntrlFifoRemainingWords();
				DPRINT2(+4,
					"priorityInversionCntDwn: = %d, fifowords: %d\n",
		...
		case SENDSYNC:	/* only the master should get this one */
			...
			if (decodeFifoWordsFlag > 0) {
				...
				execFunc("clrCumTime", NULL, NULL, NULL, NULL,
					 NULL, NULL, NULL, NULL);
			}
#endif
		...
		case EXECUTEPATTERN:
			...
			errorcode =
			    getPattern(pat_index, &patternList, &patternSize);
			if (errorcode == 0) {
				if (patternSize > 0) {
					if (patternSize < PATTERN_LOW_LIMIT)
						writeCntrlFifoBuf(patternList,
								  patternSize);
					else
						sendCntrlFifoList(patternList,
								  patternSize,
								  ONE_TIME);
				} else
					DPRINT1(-2,
						"Zero or Negative Pattern Size - Be Very Alarmed %d",
						pat_index);
			} else {
				return (errorcode);
			}
		...
		case MULTIPATTERN:
			...
			nrem = *p2CurrentWord++;	/* seqment thru the
							 ...
							 * quit.. */
			DPRINT3(-1,
				"Multi PATTERN patIndex: %d, passes of pat: %d, remaining portion size:%d \n",
		...
		case RTFREQ:
			...
			l = rtVar[i];
			DPRINT3(-1,
				" RTFREQ: %s dereferenced to %d, tsize =%d\n",
				rt_descr[i], l, k);
		...
		case ENDNVLOOP:
			...
			if (rtVar[nvLoopCntrInd[loopStkPos]] <
			    rtVar[nvLoopCountInd[loopStkPos]]) {
		...
		case VDELAY:
...
#ifdef GRADIENT_CNTLR
			{
				...
				while (lscratch1.lword >= 4L) {
					if (lscratch1.lword < 640L) {	/* 8 usec or less */
						l = lscratch1.lword;
						writeCntrlFifoWord(LATCHKEY |
								   DURATIONKEY |
								   l);
						/* DPRINT1(2,"output normal = %lx\n",l); */
					} else {	/* always leave a little residual room  */
						if (lscratch1.lword >
						    320 * 65534) {
							l = 320 * 65530;
							...
							writeCntrlFifoWord
							    (LATCHKEY |
							     (17 << 26) | (65530
									   ...
									   10) |
							     320);
						} else {	/* it is smaller than 320*65530 but larger than 640 */
							repeats =
							    lscratch1.lword /
							    320;
		...
		case TASSIGN:
			...
			DPRINT5(-1,
				" TASSIGN table i %d; j= %d k= %d l= %d val= %d\n",
				i, j, k, l, rtVar[k]);
			if (errorcode)
				return (errorcode);
			break;
		...
		case TPUT:
			...
			l = rtVar[j];
			DPRINT5(-1,
				" TPUT    table i %d; j= %d k= %d l= %d val= %d\n",
				i, j, k, l, rtVar[k]);
		...
		case TSETPNTR:
			...
			k = *p2CurrentWord++;
			rtVar[k] =
			    (unsigned int)getP2TableElement(i, l, &errorcode,
							    "tassign");
			if (errorcode)
				return (errorcode);
			break;
		...
		case INITSCAN:
			...
			if (pAcqReferenceData->il_incr == 0) {
				...
				pAcqReferenceData->bsct =
				    pAcqReferenceData->il_incrBsCnt;
				pAcqReferenceData->ct =
				    pAcqReferenceData->il_incrBsCnt *
				    pAcqReferenceData->bs;
		...
		case NEXTSCAN:
			...
			if (pAcqReferenceData->cpf)
				...
				pAcqReferenceData->oph =
				    pAcqReferenceData->rtvptr;

			DPRINT5(+0,
				" Arg0: %d, Arg1: %d, Arg2: %d, Arg3: %d, Arg4: %d\n",
				*p2CurrentWord, *(p2CurrentWord + 1),
				*(p2CurrentWord + 2), *(p2CurrentWord + 3),
				*(p2CurrentWord + 4));

			p2CurrentWord += 5;

			if ((pAcqReferenceData->ct == 0)
			    && (pAcqReferenceData->ssct <= 0))
				initvalFlag = 1;
		...
		case ENDOFSCAN:
			...
			if (!channelBitsSet) {
				pCurrentStatBlock->AcqChannelBitsConfig1 =
				    *p2CurrentWord++;
				pCurrentStatBlock->AcqChannelBitsConfig2 =
				    *p2CurrentWord++;
				pCurrentStatBlock->AcqChannelBitsActive1 =
				    *p2CurrentWord++;
				pCurrentStatBlock->AcqChannelBitsActive2 =
				    *p2CurrentWord++;
				channelBitsSet = 1;
			} else
				p2CurrentWord += 4;
...
//#endif
			DPRINT2(+2,
				"A32Interp(): il_flag: %d , Acode residentFlag: %d \n",
			...
			if ((il_flag == 1) && (residentFlag == 1)) {
				DPRINT2(+2, "NEXTCODESET: ct: %ld , nt: %ld \n",
					pAcqReferenceData->ct,
					pAcqReferenceData->nt);
				if (pAcqReferenceData->ct ==
				    pAcqReferenceData->nt) {
					markAcodeSetDone(expName,
							 curAcodeNumber);
...
#ifdef  DURATION_DECODE
			if (decodeFifoWordsFlag > 0) {
				execFunc("prtFifoTime", "NEXTCODESET", NULL,
					 NULL, NULL, NULL, NULL, NULL, NULL);
				execFunc("clrFidTime", NULL, NULL, NULL, NULL,
					 NULL, NULL, NULL, NULL);
			}
#endif
...
#ifdef  DURATION_DECODE
			if (decodeFifoWordsFlag > 0) {
				execFunc("prtFifoTime", "END_PARSE", NULL, NULL,
		...
		case SHIMAUTO:
			{
				...
				len = *p2CurrentWord++;
				if ((shimmode & 1) &&
				    chkshim((int)whenshim,
					    (int)pAcqReferenceData->ct,
					    &tmpSampleHasChanged)) {
#ifdef MASTER_CNTLR
		...
		case INITDDR:
			{
				...
				DPRINT5(-1,
					"INITDDR: fids:%d fidBuffs:%lu fidBuffSize:%lu NF:%lu NFMOD:%lu\n",
					fidDim, fidBuffs, fidBuffSize, fidNF,
					fidNFMod);

// TSPRINT("dataInitial() start:");
				/* initialize the dataObj, to asllocat the fidstablock for acquisition and uplink */
				if (dataInitial
				    (pTheDataObject, fidBuffs,
				     fidBuffSize) == ERROR) {
					errorcode = HDWAREERROR + INVALIDACODE;
					return (errorcode);
				}
...
// TSPRINT("dataInitial() ddr_set_exp:");
		...
		case NEXTSCANDDR:
			{
				...
				if (avar >= 0) {
					DPRINT1(-1,
						"NEXTSCANDDR:  vvvvvvvvvvvvvvvvvvvvvv acqval=%d\n",
						(int)rtVar[avar]);
				} else if (skip) {
					DPRINT1(-1,
						"NEXTSCANDDR:  vvvvvvvvvvvvvvvvvvvvvv sscnt=%d\n",
						pAcqReferenceData->ssct);
				} else {
					DPRINT1(-1,
						"NEXTSCANDDR:  vvvvvvvvvvvvvvvvvvvvvv args=%d\n",
						args);
				}
				...
				if (skip) {	// just put out a delay for pinsync and exit
					...
					cnt =
					    fifoEncodeGates(0, mask, gates,
							    &(instwords[0]));
					cnt =
					    cnt + fifoEncodeDuration(1,
								     psonTicks,
								     &(instwords
								       [cnt]));
					cnt =
					    cnt + fifoEncodeGates(0, mask, 0,
								  &(instwords
								    [cnt]));
					writeCntrlFifoBuf((int *)instwords,
							  cnt);
					DPRINT4(+1,
						"mask: 0x%lx gates: 0x%lx, cnt: %d, instwrd: 0x%lx\n",
						mask, gates, cnt, instwords[0]);

					if (pAcqReferenceData->ssct > 0) {
						DPRINT(-1,
						       "NEXTSCANDDR: ^^^^^^^^^^^^^^^ early exit <sscnt>\n");
					} else {
						DPRINT(-1,
						       "NEXTSCANDDR: ^^^^^^^^^^^^^^^ early exit <acqvar>\n");
					}
					break;
				}
				pAcqReferenceData->cfct = cf =
				    (cf % fidNF) ? cf : 0;

				DPRINT6(0,
					"NEXTSCANDDR:  np:%d nt:%d ct:%d bs:%d ix:%d cf:%d\n",
					np, nt, ct, bs, ix, cf);
				newblock =
				    nextScan(pAcodeId, ix, fidNF, fidNFMod, cf,
					     fidBuffSize, &srctag, &dsttag);
				...
				mode |= ((ix == fidDim)
					 && (pAcqReferenceData->ct == (nt - 1))
					 && (cf ==
					     fidNF - 1)) ? EXP_LAST_SCAN : 0;

				DPRINT6(0,
					"NEXTSCANDDR: ix %d:%d ct %d:%d cf %d:%d\n",
					ix, fidDim, pAcqReferenceData->ct,
					nt - 1, cf, fidNF - 1);
				...
				DPRINT4(-1,
					"NEXTSCANDDR: mode: 0x%x src: %d dst: %d nt: %d\n",
				...
				if (newblock) {
					DPRINT3(+0,
						"NEXTSCANDDR: nt: %ld, ct: %ld, remaining ct: %ld\n",
						nt, pAcqReferenceData->ct,
						rem_ct);
					if (bs > 0)
						endct =
						    (rem_ct < bs) ? rem_ct : bs;
				...
				if (cf == 0)
					DPRINT3(-1,
						" NEXTSCANDDR:  ct %d started src:%ld dst:%ld\n",
						pAcqReferenceData->ct, srctag,
						dsttag);

				DPRINT3(-1,
					" NEXTSCANDDR:  FidNum: %ld,  srctag: %ld, dsttag: %ld \n",
					ix, srctag, dsttag);
				if (!ddrstarted || lastsrc != srctag
				    || lastdst != dsttag || endct != lastnt) {
					ddr_set_acm(srctag, dsttag, endct);
					lastsrc = srctag;
					lastdst = dsttag;
					lastnt = endct;
					DPRINT3(+0,
						" NEXTSCANDDR:  +++>> ddr_set_acm(%ld,%ld,%ld)\n",
						srctag, dsttag, endct);
				}
				...
				if (pvar >= 0) {
					...
					DPRINT3(+1,
						" NEXTSCANDDR:  pvar(%d)*pstep(%g)=%g\n",
						rtVar[pvar], phaseStep,
						rtVar[pvar] * phaseStep);
				}
				...
				if (!ddrstarted || lastp != recvPhase) {
					...
					lastp = recvPhase;
					DPRINT1(+0,
						" NEXTSCANDDR:  +++>> ddr_set_rp(%lf)\n",
						recvPhase);
				}
				DPRINT2(+0,
					" NEXTSCANDDR:  +++>> ddr_push_scan(%ld,0x%0.8X)\n",
					srctag, mode);
				...
				if ((newFid && pAcqReferenceData->cfct == 1)
				    || ((newblock == 1) && (il_flag == 1)
					&& (cf == 0))) {
				...
				DPRINT3(+0,
					"NEXTSCANDDR: newFid: %d, gates: 0x%lx, pson delay: %d ticks\n",
					newFid, pinSyncGates, psonTicks);
				cnt =
				    fifoEncodeGates(0, mask, gates,
						    &(instwords[0]));
				DPRINT3(+1,
					"mask & gates: 0x%lx, cnt: %d, instwrd: 0x%lx\n",
					gates, cnt, instwords[0]);
				cnt =
				    cnt + fifoEncodeDuration(1, psonTicks,
							     &(instwords[cnt]));
				...
				if (!ddrstarted) {
					ddrstarted = 1;
					DPRINT1(+0,
						"NEXTSCANDDR:  +++>> ddr_start_exp(0x%X)\n",
						mode);
					ddr_start_exp(mode);
				}
				DPRINT(-1,
		...
		case SEND_ZERO_FID:
			{
				...
				if (avar >= 0) {
					DPRINT1(-1,
						"SEND_ZERO_FID:  vvvvvvvvvvvvvvvvvvvvvv acqval=%d\n",
						(int)rtVar[avar]);
					break;
				} else if (skip) {
					DPRINT1(-1,
						"SEND_ZERO_FID:  vvvvvvvvvvvvvvvvvvvvvv sscnt=%d\n",
						pAcqReferenceData->ssct);
					break;
				} else {
					DPRINT2(-1,
						"SEND_ZERO_FID:  vvvvvvvvvvvvvvvvvvvvvv ix=%d, syncFlag=%d\n",
						ix, syncFlag);
				}
				...
				if (newblock == 1) {
					...
					if (lastsrc != srctag
						...
						lastnt = endct;
						DPRINT3(+0,
							" SEND_ZERO_FID:  +++>> srctag:%ld, dsttag: %ld, endct: %ld\n",
							srctag, dsttag, endct);
					}
					pAcqReferenceData->cfct++;

					pStatBlk =
					    dataGetStatBlk(pTheDataObject,
							   srctag);
					...
					DPRINT4(-1,
						"SEND_ZERO_FID:  ix: %ld, Fid: %ld, Tag: %ld, sync: %d\n",
						ix, pStatBlk->elemId, srctag,
						syncFlag);
					if (syncFlag) {
		...
		case SETACQ_DDR:	// set or clear DDR_ACQ gate
			{
				...
				if (j == 0) {
					DPRINT(0,
					       "SETACQ_DDR: clearing DDR_ACQ\n");
					gate = 0;
				} else {
					if (skip) {
						DPRINT(0,
						       "SETACQ_DDR: set DDR_ACQ called in dummy scan <aborting>\n");
						break;
					}
					DPRINT(0,
					       "SETACQ_DDR:  setting DDR_ACQ\n");
					gate = DDR_ACQ;
				}
				cnt =
				    fifoEncodeGates(0, mask, gate,
						    &(instwords[0]));
				writeCntrlFifoBuf((int *)instwords, cnt);
				DPRINT4(1,
					"mask: 0x%lx gates: 0x%lx, cnt: %d, instwrd: 0x%lx\n",
		...
		case VRFAMPS:
			...
			DPRINT1(-4, "VRFAMPS TPWR = 0x%x\n",
				((rtVar[k] * i) & 0xffff));
			writeCntrlFifoWord(RFAMPSCALEKEY |
					   ((rtVar[k] * i) & 0xffff));
			break;
		...
		case SELECTRFAMP:
			...
			selectRFAmp(i);
			DPRINT1(-1,
				"SELECTRFAMP: called selectRFAmp() with arg %d\n",
				i);
			break;
		...
		case DECPROGON:
			...
			DecInfo.savedPhase =
			    (smallPhaseWord + quadPhaseWord) & 0xffff;
		...
		case DECPROGOFF:
			...
			if (k == 1) {	/* sync type */
				...
				lscratch2.lword =
				    (DecInfo.patTicks / DecInfo.numReps);
...
*/
					decSkipTicks =
					    DecInfo.skipTicks * jtemp;
				} else
					errLogRet(LOGIT, debugInfo,
						  "invalid decoupler asynchroziation scheme\n");

				if (decSkipTicks >= DecInfo.patTicks)
					decSkipTicks =
					    decSkipTicks % DecInfo.patTicks;

				DPRINT4(1,
					"DECPROGOFF async: %d  DecInfo.skipTicks=%lld  decSkipTicks=%lld  DecInfo.patTicks=%lld\n",
					l, DecInfo.skipTicks, decSkipTicks,
					DecInfo.patTicks);
				status =
				    progDecSkipOff(i, j, (lscratch1.lword),
						   decSkipTicks);
		...
		case TINFO:
			...
			DPRINT3(-1, "RFINFO band=%d, xmtr=%d, power=%d",
				rfInfo.ampHiLow, rfInfo.xmtrHiLow,
				rfInfo.tunePwr);
			break;
		...
		case SETVGRDROTATION:
			...
			diagPrint(debugInfo, "\n");
			diagPrint(debugInfo,
				  "pattern id = %d   rtvar name=%d   rtvar value=%d   index=%d\n",
		...
		case SETPEVALUE:
			...
			if (i == 4) {
				DPRINT4(-1, "SASSETMYPE %x %x  %x  %x\n", i,
					d_peR_step, d_peR_vind, d_peR_lim);
			...
			if (i == 5) {
				DPRINT4(-1, "SASSETMYPHASE %x %x  %x  %x\n", i,
					d_peP_step, d_peP_vind, d_peP_lim);
			...
			if (i == 6) {
				DPRINT4(-1, "SASSETMYSLICE %x %x  %x  %x\n", i,
					d_peS_step, d_peS_vind, d_peS_lim);
		...
		case OBLPEGRD:
			...
			statGx =
			    srotm_11 * G_r + srotm_12 * G_p + srotm_13 * G_s;
			statGy =
			    srotm_21 * G_r + srotm_22 * G_p + srotm_23 * G_s;
			statGz =
			    srotm_31 * G_r + srotm_32 * G_p + srotm_33 * G_s;
			...
			if ((Gx > 32767) || (Gx < -32767)) {
				errLogRet(LOGIT, debugInfo,
					  "X gradient value %d is outside +/-32767 range\n",
					  Gx);
				markAcodeSetDone(expName, curAcodeNumber);
				errorcode =
				    HDWAREERROR + GRADIENT_AMPL_OUTOFRANGE;
				return (errorcode);
			}

			if ((Gy > 32767) || (Gy < -32767)) {
				errLogRet(LOGIT, debugInfo,
					  "Y gradient value %d is outside +/-32767 range\n",
					  Gy);
				markAcodeSetDone(expName, curAcodeNumber);
				errorcode =
				    HDWAREERROR + GRADIENT_AMPL_OUTOFRANGE;
			...
			if ((Gz > 32767) || (Gz < -32767)) {
				errLogRet(LOGIT, debugInfo,
					  "Z gradient value %d is outside +/-32767 range\n",
					  Gz);
				markAcodeSetDone(expName, curAcodeNumber);
				errorcode =
				    HDWAREERROR + GRADIENT_AMPL_OUTOFRANGE;
		...
		case SHAPEDGRD:
			...
			errorcode =
			    getPattern(pat1_index, &patternList1,
				       &patternSize1);
			...
			if ((errorcode == 0) && (patternSize1 > 0)) {
				...
				for (m = 0; m < patternSize1; m++) {
					...
					if ((Gx > 32767) || (Gx < -32767)) {
						errLogRet(LOGIT, debugInfo,
							  "gradient value %d is outside +/-32767 range in shapedgradient\n",
							  Gx);
						markAcodeSetDone(expName,
								 curAcodeNumber);
						errorcode =
						    HDWAREERROR +
						    GRADIENT_AMPL_OUTOFRANGE;
						return (errorcode);
					}
		...
		case OBLPESHAPEDGRD:
			...
			DPRINT3(-1,
				"pat1index: %d pat2index: %d pat3index: %d\n",
			...
			if ((i & 0x8) || (i & 0x10) || (i & 0x20)) {	// POP off the extra codes
				...
				DPRINT3(-1,
					"SAS pat4index: %d pat5index: %d pat6index: %d\n",
			...
			if ((errorcode == 0) && (i & 0x2)) {
				errorcode =
				    getPattern(pat2_index, &patternList2,
					       &patternSize2);
				if (errorcode == 0) {
					pIntW2 = patternList2;
					ii = patternSize2;
			...
			if ((errorcode == 0) && (i & 0x4)) {
				errorcode =
				    getPattern(pat3_index, &patternList3,
					       &patternSize3);
				if (errorcode == 0) {
					pIntW3 = patternList3;
					ii = patternSize3;
			...
			if (errorcode == 0) {
				if (ii > 0)	// pattern size
				{
					...
					if (pgwfg == NULL)
						errLogRet(LOGIT, debugInfo,
							  "OBLPESHAPED waveform array allocation is null\n");
...
*/
		...
		case SETVGRDANGLIST:
			...
			diagPrint(debugInfo,
				  "SETVGRDANGLIST: pat_index=%d pat_size=%d i=%d j=%d k=%d executed\n",
		...
		case EXEVGRDROTATION:
			...
			calc_obl_matrix(ang_psi, ang_phi, ang_theta, &tmp_m11,
					&tmp_m12, &tmp_m13, &tmp_m21, &tmp_m22,
					&tmp_m23, &tmp_m31, &tmp_m32, &tmp_m33);
		...
		case SHAPEDGRD:
			...
			if ((errorcode == 0) && (patternSize1 > 0)) {
				...
				if (ii > 0)
					gradgridword[kk++] =
					    (LATCHKEY | DURATIONKEY | ii);

				/* kk is the number of words written into gradgrid array */

				DPRINT1(-1,
					"SHAPEDGRD: no of words in gradgrid array = %d\n",
					kk);
				...
				if (pgwfg == NULL)
					errLogRet(LOGIT, debugInfo,
						  "SHAPED waveform array allocation is null\n");
		...
		case OBLPESHAPEDGRD:
			...
			if (errorcode == 0) {
				if (ii > 0)	// pattern size
				{
					...
					DPRINT1(-1,
						"OBLPESHAPEDGRD: no of words in gradgrid array = %d\n",
						kk);
						...
						if (sim_gradient) {
							...
							if (d_peR_step != 0) {
								...
								if ((*(pIntW4 - 1) == 32767) || (*(pIntW4 - 1) == -32767))	// original mode for PE truncate for constant part of WFG
								{
									tempGx1
									    =
									    ((d_srotm_11 * d_peR_step + 0x3fff) >> 15) * rtVar[d_peR_vind];
									tempGy1
									    =
									    ((d_srotm_21 * d_peR_step + 0x3fff) >> 15) * rtVar[d_peR_vind];
									tempGz1
									    =
									    ((d_srotm_31 * d_peR_step + 0x3fff) >> 15) * rtVar[d_peR_vind];
								} else	// new round off before PE mult for varying part of WFG
								{
									tempv =
									    d_peR_step
							...
							if (d_peP_step != 0) {
								if ((*(pIntW5 - 1) == 32767) || (*(pIntW5 - 1) == -32767))	// original mode for PE truncate for constant part of WFG
								{
									tempGx2
									    =
									    ((d_srotm_12 * d_peP_step + 0x3fff) >> 15) * rtVar[d_peP_vind];
									tempGy2
									    =
									    ((d_srotm_22 * d_peP_step + 0x3fff) >> 15) * rtVar[d_peP_vind];
									tempGz2
									    =
									    ((d_srotm_32 * d_peP_step + 0x3fff) >> 15) * rtVar[d_peP_vind];
								} else	// new round off before PE mult for varying part of WFG
								{
									tempv =
									    d_peP_step
							...
							if (d_peS_step != 0) {
								if ((*(pIntW6 - 1) == 32767) || (*(pIntW6 - 1) == -32767))	// original mode for PE truncate for constant part of WFG
								{
									tempGx3
									    =
									    ((d_srotm_13 * d_peS_step + 0x3fff) >> 15) * rtVar[d_peS_vind];
									tempGy3
									    =
									    ((d_srotm_23 * d_peS_step + 0x3fff) >> 15) * rtVar[d_peS_vind];
									tempGz3
									    =
									    ((d_srotm_33 * d_peS_step + 0x3fff) >> 15) * rtVar[d_peS_vind];
								} else	// new round off before PE mult for varying part of WFG
								{
									tempv =
									    d_peS_step
		...
		case ROLLCALL:
			{
				...
				strncpy(secretword, (char *)p2CurrentWord,
					i * 4);
				DPRINT1(-2,
					"ROLLCALL: Controllers used in PS: '%s'\n",
					(char *)secretword);
				...
				for (retryCntDwn = 3; retryCntDwn > 0;
				     retryCntDwn--) {
					mcntlrs[0] = 0;
					nmissing =
					    cntlrSet2UsedInPS(secretword,
							      mcntlrs);
					if (nmissing == 0) {
						ok = 1;
						/* p2CurrentWord += i; */
				...
				if (!ok) {
					DPRINT(-1,
					       "ROLLCALL: FAILED to Find required Controllers\n");
					DPRINT2(-1,
						"ROLLCALL: %d missing, MIAs: '%s'\n",
						nmissing, mcntlrs);
					sendExceptionViaParsCom(WARNING_MSG,
								WARNINGS +
								MISSINGCNTLRS,
								0, 0, NULL);
				} else {
					DPRINT(-1,
					       "ROLLCALL: All required Controllers present.\n");
				}
				 ...
				 */
				DPRINT4(2,
					"real send2AllCntlrs: CNTLR_CMD_APARSER, numAcodes: %d, nTables: %d, startFid: %d, expid: '%s'\n",
					pAcodeId->num_acode_sets,
					pAcodeId->num_tables,
					pAcodeId->cur_acode_set, pAcodeId->id);
				...
				send2AllCntlrsViaParsCom(CNTLR_CMD_APARSER,
							 pAcodeId->
							 num_acode_sets,
							 pAcodeId->num_tables,
							 pAcodeId->
			...
			*p2CurrentWord++;
			{
				...
				oneDac[1] = 1;
				oneDac[2] = *p2CurrentWord;
				pCurrentStatBlock->AcqShimValues[1] =
				    *p2CurrentWord++;
			...
			Send2LockViaParsCom(LK_SET_GAIN, *p2CurrentWord++, 0,
					    0.0, 0.0);
			...
			if (!tnlk_flag) {
				...
				Send2LockViaParsCom(LK_SET_POWER,
						    *p2CurrentWord, 0, 0.0,
						    0.0);
			}
			*p2CurrentWord++;
			...
			Send2LockViaParsCom(LK_SET_PHASE, *p2CurrentWord++, 0,
					    0.0, 0.0);

			pCurrentStatBlock->AcqLockFreq1 = lscratch1.nword[0] =
			    *p2CurrentWord++;
			pCurrentStatBlock->AcqLockFreq2 = lscratch1.nword[1] =
			    *p2CurrentWord++;
			DPRINT3(-1, "lockfreq = %f 0x%x 0x%x\n",
				lscratch1.dword, lscratch1.nword[0],
				lscratch1.nword[1]);
			//send2Lock(LK_SET_FREQ, lscratch1.nword[0], lscratch1.nword[1],
			//                       lscratch1.dword, 0.0);
			Send2LockViaParsCom(LK_SET_FREQ, lscratch1.nword[0],
					    lscratch1.nword[1], lscratch1.dword,
					    0.0);

			DPRINT1(-1, "lock mode word = 0x%x\n", *p2CurrentWord);
			saveTcValues((*p2CurrentWord & 0xFF),
				     ((*p2CurrentWord >> 8) & 0xFF));
		...
		case MRIUSERBYTE:
			{
				...
				DPRINT3(1, "MRIUSERBYTE - rw=%d rtVar[%d]=%d",
					rw, i, rtVar[i]);
		...
		case GETSAMP:
			{
				...
				queueSamp(GETSAMP, sample2ChgTo, skipsampdetect,
					  pAcodeId, bumpFlag);
				DPRINT(0,
		...
		case LOADSAMP:
			{
				...
				bumpFlag = *p2CurrentWord++;
				queueLoadSamp(LOADSAMP, sample2ChgTo,
					      skipsampdetect, pAcodeId,
					      spinActive, bumpFlag);
				DPRINT(0,
...
#define MAX(a,b) ((a) > (b) ? (a) : (b))
				...
				int corrected =
				    value < 0 ? value + zgradcorrlo : value >
				    0 ? zgradcorrhi + value : 0;
				int capped = corrected < 0 ? MAX(corrected,
								 SHRT_MIN) :
				    MIN(corrected,
					SHRT_MAX);
...
void sendInfo2ConsoleStat(int tickmismatch)
{
	...
	pCurrentStatBlock->AcqChannelBitsActive2 = 0;
	DPRINT(1,
	       "sendInfo2ConsoleStat: set active channel bits to 0 & send to ConsoleStat\n");
#endif
}
