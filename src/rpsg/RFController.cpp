...
void RFController::setVQuadPhase(int rtvar)
{
   ...
   if (rtvar >= BASEINDEX)
   {
     ...
     if (psgFile) fprintf(psgFile,"TASSIGN:\n  table: %d\n  index: %d\n  dest: %d\n",
           rtvar-BASEINDEX, buffer[1],  buffer[2]);
...
cPatternEntry *RFController::resolveRFPattern(char *nm, int flag, const char *emsg, int action)
{
    ...
    wcount = 0; eventcount = 0;
       ...
       if (i != 1)
	 {
        ...
        if (l > 0)
	  {
          ...
          //  fprintf(stdout,"putPat: phase %g\n",f1);
             ...
             if (repeats > 0)
	       {
           fprintf(stdout,"putPat: repeats %d\n",repeats);
     ...
     if (eventcount > 0)
     {
        ...
        cout << "Pattern entry: name " << nm << " flag= " << flag << " count= " << eventcount << " wcount= " << wcount << endl;
...
// ampmode is RF_CW_MODE
  ...
  if (psgFile) fprintf(psgFile,"FREQ:\n  rfch: %d\n  MHz: %g\n", logicalID, baseFreq+baseOffset);
