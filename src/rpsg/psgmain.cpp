...
sleep(30);
   ...
   if (!dps_flag)
   {
      char yamlpath[MAXPATHL];		/* file path for Codes */
      strcpy(yamlpath,filename);
      strcat(yamlpath,".yaml");
      if ( (psgFile = fopen(yamlpath,"w")) == NULL )
      {
         abort_message("go: could not create file %s for pulse sequence codes\n", yamlpath);
      }
   }
...
/* write INIT code for "go" or "su" for various controllers */
  ...
  sync();
#endif
  if (!acqiflag && !option_check("forcePSG") && QueueExp(filexpath,nextflag))
      psg_abort(1);
...
void PSGGo(double arraydim)
{
   ...
   if (psgFile) fprintf(psgFile,"ARRAYDIM: %d\n", (int) (arraydim+0.1));
   if (arraydim <= 1.5)
   {
      ix = 1;
      pre_expsequence();
