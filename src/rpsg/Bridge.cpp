...
cout << " Bridge delay" << endl;
   ...
   if (psgFile) fprintf(psgFile,"DELAY: %f\n",dur);
}
...
void no_shim_delay(double dur)
{
   if(dur<=0)
       return;
cout << "Bridge no_shim_delay" << endl;
   long long count;
   Controller *tmp;
   if (P2TheConsole->isParallel())
      tmp = P2TheConsole->getParallelController();
...
cout << "Bridge hsdelay" << endl;
   ...
   if ( (hs[index] == 'y') || (hs[index] =='Y') )
   {
       ...
       else
       {
          ...
          tmp->outputACode(HOMOSPOIL,1,codeStream);    // 4 clocks
          tmp->noteDelay(8);
          if (psgFile) fprintf(psgFile,"HOMOSPOIL: %f\n",hst);
       }
   }
...
CEXTERN void startacq_rcvr(double tm)
{
  int loChNum;
  long long ticks, ticks0;
cout << "Bridge startacq_rcvr" << endl;
  ticks = 0LL; ticks0 = 0LL;
  DDRController *ddrCh;
  RFController *observeCh;
  int numActiveDDRs = 0;
...
cout << "Bridge dfltstdacq " << endl;
  ...
  for(int i=0; i<P2TheConsole->numDDRChan; i++)
  {
    ...
    {
      if (psgFile) fprintf(psgFile,"ACQUIRE:\n  alfa: %f\n  np: %d\n  dwell: %f\n", alfa, (int) np, 1.0/sw);
      if ( !doneOnce )
      {
         cout << "Acquire blank amp" << endl;
        observeCh->blankAmplifier();
         cout << "Acquire gate RF off amp" << endl;
        observeCh->setGates(RFGATEOFF(RF_TR_GATE));
         cout << "Acquire call ddr acquire" << endl;
        ticks = ddrCh->Acquire();
        if (firstDDR == 1)
        {
          ticks0 = ticks;
        ...
        if (ticks != ticks0)
          abort_message("time tick mismatch between DDR Controllers. abort!\n");
         cout << "Acquire set tick delay" << endl;
        observeCh->setTickDelay(ticks);
         cout << "Acquire set RF gate on" << endl;
        observeCh->setGates(RFGATEON(RF_TR_GATE));
        if ( P2TheConsole->getRcvrsConfigMap() == MULTIMGRCVR_MULTACQ )
          doneOnce = 1;
      }
...
void dfltacq()
{
  ...
  if (homodecflag)
  {
    abort_message("acquisition during homo decoupling not supported. abort!\n");
...
cout << "Bridge genRFPulse " << endl;
   ...
   P2TheConsole->update4Sync(ticks);
   if (psgFile)
      fprintf(psgFile,"PULSE:\n  rfch: %d\n  pw: %f\n  phase: %d\n  rx1: %f\n  rx2: %f\n",
           rfch, pw, phaseC, rof1, rof2);
}
...
void genRFShapedPulse(double pw, char *name, int rtvar, double rof1, double rof2, double g1, double g2, int rfch)
{
   ...
   if (pw <= 0.0)
     return;
cout << "Bridge genRFShapedPulse " << endl;
   value = pw;
   ...
   cout << "Set duration per state " << hex << duration << dec << endl;
   cout << "Set duration per state cntrl " << hex << (DURATIONKEY | duration) << dec << endl;
   ...
   if (psgFile) fprintf(psgFile,"SHAPEPULSE: %s\n  rfch: %d\n  id:  %d\n  incr: %d\n  time: %g\n  phase: %d\n  rx1: %g\n  rx2: %g\n",
         name, rfch, tmp->getReferenceID(), divs,
         duration*12.5e-9, rtvar, rof1, rof2);
...
void genRFShapedPulseWithOffset(double pw, char *name, int rtvar, double rof1, double rof2, double offset, int rfch)
{
   ...
   if (pw <= 0.0)
     return;
cout << "Bridge genRFShapedPulseWithOffset " << endl;
   value = pw;
 ...
 int rfch)
{
   ...
   if (pw <= 0.0)
     return;
cout << "Bridge gen_shapedpulseoffset " << endl;
   value = pw;
...
cout << "Bridge prg_dec_on_offset " << endl;
    RFController *S1 = (RFController *) P2TheConsole->getRFControllerByLogicalIndex(rfdevice);
...
{
cout << "Bridge genBlankOnOff " << endl;
   RFController *Work = (RFController *) P2TheConsole->getRFControllerByLogicalIndex(rfch);
   if (Work->getProgDecInterlock() > 0)
   {
       if (bgflag)
...
{
cout << "Bridge genTxGateOnOff " << endl;
  RFController *Work = (RFController *) P2TheConsole->getRFControllerByLogicalIndex(rfch);
  if (Work->getProgDecInterlock() > 0)
  {
      if (bgflag)
...
{
    if (rfch > P2TheConsole->numRFChan) return;
cout << "Bridge definePhaseStep " << endl;
    RFController *Work = (RFController *) P2TheConsole->getRFControllerByLogicalIndex(rfch);
    if (Work->getProgDecInterlock() > 0)
    {
        if (bgflag)
