...
void F_initval(double value, int index)
{
   ...
   code[1] = index;
   rtinit_count++;  
   if (psgFile) fprintf(psgFile,"FRTINIT:\n  value: %d\n  index: %d\n", code[0], code[1]);
...
void init_acqvar(int index,int val)
{
   ...
   {
      code[0]=val; code[1]=index;
      if (psgFile) fprintf(psgFile,"RTINIT:\n  value: %d\n  index: %d\n", code[0], code[1]);
      broadcastCodes(RTINIT,2,code);
      rtinit_count++;
   }
   else
   {
...
void std_lc_init()
{
    ...
    if (bgflag)
    {
        fprintf(stdout,"finish np: %d, nt: %d\n",Alc->np,Alc->nt);
	fprintf(stdout,"npr_ptr offset: %d \n",npr_ptr);
        fprintf(stdout,"np: %d, nt: %d\n",Alc->np,Alc->nt);
	fprintf(stdout,"ct offset: %d \n",ct);
	fprintf(stdout,"oph offset: %d \n",oph);
	fprintf(stdout,"bsctr offset: %d \n",bsctr);
        fprintf(stdout,"rtvptr set to : %d \n",Alc->rtvptr);
        fprintf(stdout,"ctss(rtvptr) offset: %d \n",ctss);

	fprintf(stdout,"zero offset: %d \n",zero);
    }
}
