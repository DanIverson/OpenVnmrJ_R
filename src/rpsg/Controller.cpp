...
int Controller::outputACode(int Code, int many, int *stream)
{
 ...
 cout << Name << " sends ACODE "  << (Code & ~ACDKEY) << " ( " << getAcode(Code & ~ACDKEY) << " )" << endl;
   if (bgflag)
   {
     switch (Code)
     {
     case (int) DLOAD:  cout << "DLOAD with " << many << setw(8) << setfill('0') << endl;
          // cout << "RFPHASEKEY " << setw(8) <<  hex << RFPHASEKEY << dec << endl;
          // cout << "GATEKEY " << setw(8) <<  hex << GATEKEY << dec << endl;
          for (i=0; i < many; i++)
          {
             cout  << setw(5) << setfill('0') << i << "   " << setw(8) << hex << *(stream+i) << dec;
             // cout << "Check latch " << setw(8) <<  hex << (*(stream+i) & LATCHKEY) << dec << endl;
             if ( *(stream+i) & LATCHKEY )
                cout << "   Latched   ";
             else
                cout << "   Unlatched ";
             if ( ( *(stream+i) & AUX ) == AUX )
                cout << "AUX   ";
             else if ( ( *(stream+i) & RFUSERKEY ) == RFUSERKEY )
                cout << "RF user   ";
             else if ( ( *(stream+i) & RFAMPSCALEKEY ) == RFAMPSCALEKEY )
                cout << "RF amp scale  ";
             else if ( ( *(stream+i) & RFAMPKEY ) == RFAMPKEY )
                cout << "RF amp   ";
             else if ( (*(stream+i) & RFPHASECYCLEKEY) == RFPHASECYCLEKEY )
                cout << "RF phase cycle  ";
             else if ( (*(stream+i) & RFPHASEKEY) == RFPHASEKEY )
                cout << "RF phase   ";
             else if ( ( *(stream+i) & DURATIONKEY ) == DURATIONKEY )
                cout << "duration   ";
             else if ( (*(stream+i) & GATEKEY ) == GATEKEY )
                cout << "gate   ";
             else
                cout << "unknown   ";
             cout << endl;
...
{
#ifdef CORE
   cout << "add2stream: " << setw(8) << hex << dataWord << dec << " on " << Name  << endl;
   if ( dataWord == 0x1800ffff )
   {
      int* intptr = NULL;
      // *intptr = 10;
      cout << "add2stream: core dump case " << endl;
   }
#endif
