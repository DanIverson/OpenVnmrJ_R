...
void printlpel()
{
   ...
   for (i = 0; i < MAXLOOPS; i++)
   {
      if (lpel[i])
      {
         ...
         num = lpel[i]->numvar;
         fprintf(stdout, "lpel[%d] contains: numvar= %d with numvalues= %d\n",
	      i, lpel[i]->numvar, lpel[i]->numvals);
         for (j=0; j<num; j++)
         {
            fprintf(stdout, "Loop element %d: Variable[%d] '%s'\n",
	      i, j, lpel[i]->lpvar[j]);
         }
      }
   }
...
int ntArray()
{
   ...
   if ( ! P_getreal(CURRENT, "ntarray", &val, 1) )
   {
      if (val < 0.9)
      {
         fprintf(stdout,"nt array is false because ntarray parameter not = 1\n");
         return(0);
      }
   }
   else
   {
      fprintf(stdout,"nt array is false because ntarray parameter does not exist\n");
      return(0);
   }
   for (i = 0; i < MAXLOOPS; i++)
   {
   ...
   if (ntA >= 0)
   {
      fprintf(stdout,"nt found numvals=%d\n", lpel[ntA]->numvals);
      for (i=1; i <= lpel[ntA]->numvals; i++) 
      {
	      if ( ! P_getreal(CURRENT, "nt", &val, i) )
         {
            fprintf(stdout,"nt[%d]=%g\n", i, val);
            if ( (int) val != 1 )
            {
               fprintf(stdout,"nt array is false because not all = 1\n");
               return(0);
            }
         }
      }
      fprintf(stdout,"nt array is true\n");
      return(1);
   }

   fprintf(stdout,"nt array is false\n");
   return(0);
}
...
void arrayPS(int index, int numarrays, int arrayDim)
{
   ...
   fprintf(stdout,"arrayPS index=%d numarray=%d arrayDim=%d\n", index, numarrays, arrayDim);
   name = lpel[index]->lpvar[0];
   nvals = lpel[index]->numvals;
   if ( (ntIndex >= 0) && ! strcmp(name,"nt") )
      ntelem = 1;
   fprintf(stdout,"ntIndex= %d ntelem=%d\n",ntIndex, ntelem);
   ...
   fprintf(stdout,"Loop over %s %d times\n", name, nvals);
   for (valindx = 1; valindx <= nvals; valindx++)
   {
      if (ntelem)
      {
         ntIndex = valindx;
         fprintf(stdout,"1: set ntIndex= %d\n",ntIndex);
      }
      ...
      for (varindx = 0; varindx < nvars; varindx++)
      {
         ...
         {
            ntIndex = valindx;
            fprintf(stdout,"2: set ntIndex= %d\n",ntIndex);
         }
...
*/
	 ...
	 ix++;			/* generating Acode for FID ix */
      ...
      if (psgFile) fprintf(psgFile,"INITSCAN:\n  ix: %d\n  np: %d\n  nt: %d\n",
                   ix, (int) np, (int) nt);
         initscan();

	 /* update lc elemid = ix */
         fprintf(stdout,"************** Call createPS ntIndex=%d\n", ntIndex);
	  createPS(arrayDim);
      ...
      if (psgFile) fprintf(psgFile,"ENDSCAN:\n");
      endofscan();
